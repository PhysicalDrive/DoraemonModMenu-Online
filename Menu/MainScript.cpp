...
namespace Chim
{
	...
	void MainScript::Initialize()
	{
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Local", SubmenuLocal, [](RegularSubmenu* sub)
		{
			...
			sub->AddOption<BoolOption<bool>>("Drug Mode", nullptr, &g_LocalFeatures->m_DrugMode);
			sub->AddOption<BoolOption<bool>>("Tiny Ped", nullptr, &g_LocalFeatures->m_Tiny);
			sub->AddOption<RegularOption>("Clone Ped", nullptr, []
			{
				PED::CLONE_PED(PLAYER::PLAYER_PED_ID(), true, false, true); // idk why you want this
			});
		});
		...
		{
			sub->AddOption<BoolOption<bool>>("Ignored By Peds", nullptr, &g_LocalFeatures->m_ignore);
		});
		...
		{
			sub->AddOption<BoolOption<bool>>("Money Drop", nullptr, &g_LocalFeatures->m_moneydrop);
		    sub->AddOption<BoolOption<bool>>("RP Drop", nullptr, &g_LocalFeatures->m_rpdrop);
		});
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("Drops", SubmenuDrop, [](RegularSubmenu* sub)
		{
			sub->AddOption<BoolOption<bool>>("Money Drop", nullptr, &g_LocalFeatures->m_moneydrop);
		    sub->AddOption<BoolOption<bool>>("RP Drop", nullptr, &g_LocalFeatures->m_rpdrop);
		});
		...
		{
			sub->AddOption<SubOption>("Drops", nullptr, SubmenuDrop);
		});
	}
