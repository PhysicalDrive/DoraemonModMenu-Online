...
BOOL DllMain(HINSTANCE hInstance, DWORD reason, LPVOID)
{
	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		CreateThread(nullptr, 0, [](LPVOID) -> DWORD
		{
			...
			std::string branding = " | Doraemon Menu";
			std::array<std::string, 4> Titles = {
				"Is this pasted?" + branding,
				"Another gta menu go burrrrr" + branding,
				"we live we love we lie" + branding,
				"idk anymore" + branding
			};
